			...
			if (key == L"RemoveUsedChains") {
				...
				only_strong = (value == L"true" or value[0] == L'y'
				               or value[0] == L'Y' or value == L"1");
                                ...
                                semdb_path = freeling::util::expand_filename(value);
...
double summarizer::average_scores(map<wstring, list<lexical_chain> > &chains) const {
	...
	for (set<relation*>::const_iterator it_r = used_relations.begin();
		...
		list<lexical_chain> &lexical_chains = chains[(*it_r)->label];
...
list<word_pos> summarizer::first_word(wostream &sout,
    	map<wstring, list<lexical_chain> > &chains, int num_words) const {
...
map<wstring, list<lexical_chain>> summarizer::build_lexical_chains(wostream &sout, const document &doc) const {
...
void summarizer::print_lexical_chains(map<wstring, list<lexical_chain>> &chains, wostream &sout) const {
	for (set<relation*>::const_iterator it_t = used_relations.begin(); it_t != used_relations.end(); it_t++) {
		list<lexical_chain> &lexical_chains = chains[(*it_t)->label];
...
list<const sentence*> summarizer::wp_to_sp(list<word_pos> &wp_l) const {
...
list<const sentence*> summarizer::summarize(wostream &sout, const document &doc, int num_words) const {
	...
	if (heuristic == L"FirstMostWeightedWord") {
		wp_res = first_most_weighted_word(sout, chains, num_words);
	} else if (heuristic == L"SumOfChainWeights") {
		wp_res = sum_of_chain_weights(sout, chains, num_words);
	} else {
		wp_res = first_word(sout, chains, num_words);
	}
